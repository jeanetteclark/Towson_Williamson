...
*Note: The order which you install packages is important. Functions with the same names will be masked by packages loaded in after the previous* Ex: The `select()` function from `MASS` will be masked by `select()` from `dplyar` 
```{r}
...
1. Prep the data 
  a. Filter for only 2021 or 2020 data respectivly
  b. Filter out non-living species (Standing dead, Litter, Bare ground)
  c. Drop the column that still contains the sum of cover that you used to get the average
  d. Convert from long to wide format 
  e. Change name of Deciduous and Evergreen shrub column to drop "shrubs" so there are no spaces in the name
```{r}
data_MANOVA_2008 <- Avg_func_quad %>%
  filter(year=="2008" & func_grp!="Standing dead" & func_grp!="Litter" & func_grp!="Bare ground")%>%
  select(-sum_func_quad) %>%
...
3. Post hoc tests 
  a. followed instructions from [this](https://www.reneshbedre.com/blog/manova.html#post-hoc-test) site
```{r}

posthoc_2008 <- lda(data_MANOVA_2008$plot ~ cbind(Deciduous, Evergreen, Forb, Grass, Lichen, Moss, Sedge))
