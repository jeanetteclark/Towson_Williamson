...
Then merge these groups back together into one dataset. *Make sure the number of observations in `relcov_func_08_21` matches `relcov_08_21`, but `relcov_func_08_21` should have one more variable (the new func_grp column you created),
```{r}
...
#subset data by year and living plants
  quad.avg.2021 <- subset(Avg_func_quad, year== "2021") 
  ...
  quad.avg.2018 <- subset(Avg_func_quad, year== "2018") 
...
```{r}
  ...
  Scat_prep <- Avg_func_quad %>% unite("code", plot:year, remove = FALSE)
  ...
  wide.Avg_func_quad           <- Reduce(MyMerge, list(Scat_yr_plot, Scat_func))
  
  #replace NA values with 0's
  wide.Avg_func_quad[is.na(wide.Avg_func_quad)] <- 0
  
  #change year to be a character string 
  wide.Avg_func_quad$year <- as.character(wide.Avg_func_quad$year)
  
  write.csv(wide.Avg_func_quad, file = "../Derived CSVs/LTER_06MAT_WIDE_avg_quad.csv")
```
...
wide.Avg_func_quad$plot <- factor(wide.Avg_func_quad$plot, levels = c("CT","F2","F5","F10"))

###Bet nan VS Eri Vag
ggplot(data = wide.Avg_func_quad, aes(x = `avg.1.Deciduous shrubs`, y = `avg.1.Eri vag`, color = plot)) +
...
```{r}
test1 <- subset(wide.Avg_func_quad, plot == "CT")
```

```{r}
###Bet nan VS Eri Vag (not separated by fert treatment )
ggplot(data = wide.Avg_func_quad, aes(x = `avg.1.Deciduous shrubs`, y = `avg.1.Eri vag`)) +
   geom_point()+
...
###Bet nan VS Forbs
ggplot(data = wide.Avg_func_quad, aes(x = `avg.1.Deciduous shrubs`, y = `avg.1.Forb`, color = plot)) +
  scale_color_manual(values = c("CT" = "#d8b365", 
                                "F2" = "#41b6c4",
                                "F5" = "#2c7fb8",
                                "F10" = "#253494"))+
...
###Eri vag VS Forbs
ggplot(data = wide.Avg_func_quad, aes(x = `avg.1.Forb`, y = `avg.1.Eri vag`, color = plot)) +
  scale_color_manual(values = c("CT" = "#d8b365", 
                                "F2" = "#41b6c4",
                                "F5" = "#2c7fb8",
                                "F10" = "#253494"))+
...
#PCORD_prep <- Avg_func_block %>% unite("code", plot:year, remove = FALSE)
PCORD_prep <- Avg_func_quad %>% unite("code", plot:year, remove = FALSE) #for data not-averaged across blocks
...
#--------
Avg_func_quad_2008 <- subset(Avg_func_quad, year== 2008) 
Avg_func_quad_2021 <- subset(Avg_func_quad, year== 2021) 

PCORD_prep_2008 <- Avg_func_quad_2008 %>% unite("code", year:plot, remove = FALSE)
PCORD_prep_2021 <- Avg_func_quad_2021 %>% unite("code", year:plot, remove = FALSE)
